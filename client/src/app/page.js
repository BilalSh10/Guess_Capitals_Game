...
export default function Home() {
  ...
  useEffect(() => {  
    ...
    let getData = async () => {
      let itemsData = await axios.get('https://todo-list-nodejs-website.onrender.com/items');
  ...
  let createItem = async () => {
    if(itemText.length !== 0) {
      try{
        await axios.post('https://todo-list-nodejs-website.onrender.com/item', {newItem: itemText});
  ...
  const handleCheckboxChange = async (e) => {
    let id = e.target.id;
    try{
      await axios.delete(`https://todo-list-nodejs-website.onrender.com/item/${id}`);
      setWhenToRefresh(!whenToRefresh);
    }catch(err){
